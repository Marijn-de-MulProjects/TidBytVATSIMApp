load("encoding/base64.star", "base64")
load("http.star", "http")
load("render.star", "render")
load("schema.star", "schema")

ONLINE_ICON = base64.decode("""""")
OFFLINE_ICON = base64.decode("""""")

ONLINE_2_ICON = base64.decode("""iVBORw0KGgoAAAANSUhEUgAAADIAAAAdCAYAAADoxT9SAAAACXBIWXMAAArrAAAK6wGCiw1aAAAAQ0lEQVRYhe3PQQ3AIADAQNgD/47BxBIacqegnWOvPR7w3Q74i5EaIzVGaozUGKkxUmOkxkiNkRojNUZqjNQYqXlm5ACM7gI+ZeraHQAAAABJRU5ErkJggg==""")
OFFLINE_2_ICON = base64.decode("""iVBORw0KGgoAAAANSUhEUgAAADIAAAAdCAYAAADoxT9SAAAACXBIWXMAAArrAAAK6wGCiw1aAAAAQklEQVRYhe3PAQ3AMAzAsH78Of8kLi2qbATJMzPvLHBuB/zFSI2RGiM1RmqM1BipMVJjpMZIjZEaIzVGaozUrBn5AFbMATkyR4jQAAAAAElFTkSuQmCC""")

DEFAULT_ICAO = "EHEH"

def main(config):
    NUM_OF_DEP = 0
    NUM_OF_ARR = 0

    DEL_ACTIVE = False
    GND_ACTIVE = False
    TWR_ACTIVE = False
    APP_ACTIVE = False

    suffixes = ["_DEL", "_GND", "_TWR", "_APP"]

    data = get_cachable_data("https://data.vatsim.net/v3/vatsim-data.json", 15)

    for pilot in data["pilots"]:
        if "flight_plan" in pilot and pilot["flight_plan"]:
            if "departure" in pilot["flight_plan"] and pilot["flight_plan"]["departure"] == config.str("icao", DEFAULT_ICAO):
                NUM_OF_DEP += 1
            if "arrival" in pilot["flight_plan"] and pilot["flight_plan"]["arrival"] == config.str("icao", DEFAULT_ICAO):
                NUM_OF_ARR += 1

    for controller in data["controllers"]:
        if config.str("icao", DEFAULT_ICAO) in controller["callsign"]:
            for suffix in suffixes:
                if suffix in controller["callsign"]:
                    if suffix == "_DEL":
                        DEL_ACTIVE = True
                    elif suffix == "_GND":
                        GND_ACTIVE = True
                    elif suffix == "_TWR":
                        TWR_ACTIVE = True
                    elif suffix == "_APP":
                        APP_ACTIVE = True
    if config.bool("styling"): 
        return render.Root(
            delay = 5,
            max_age = 15,
            show_full_animation = True,
            child = render.Box(
                padding = 1,
                child =
                render.Column(main_align = "center", cross_align = "center", children = [
                    render.Text(font = "tom-thumb", content = config.str("icao", DEFAULT_ICAO)),
                    render.Text(font = "tom-thumb", content = "DEP:%s|ARR:%s" % (NUM_OF_DEP, NUM_OF_ARR)),
                    render.Text(font = "tom-thumb", content = "--------------------------------"),
                    render.Row(children = [
                        render.Stack(children = [
                            render.Image(height = 9, src = ONLINE_2_ICON) if DEL_ACTIVE else render.Image(height = 9, src = OFFLINE_2_ICON),
                            render.Padding(pad = 2, child=render.Text(color = "#000000", font = "tom-thumb", content = "DEL")) if DEL_ACTIVE else render.Padding(pad = 2, child=render.Text(font = "tom-thumb", content = "DEL")),
                        ]),
                        render.Stack(children = [
                            render.Image(height = 9, src = ONLINE_2_ICON) if GND_ACTIVE else render.Image(height = 9, src = OFFLINE_2_ICON),
                            render.Padding(pad = 2, child=render.Text(color = "#000000", font = "tom-thumb", content = "GND")) if GND_ACTIVE else render.Padding(pad = 2, child=render.Text(font = "tom-thumb", content = "GND")),
                        ]),
                        render.Stack(children = [
                            render.Image(height = 9, src = ONLINE_2_ICON) if TWR_ACTIVE else render.Image(height = 9, src = OFFLINE_2_ICON),
                            render.Padding(pad = 2, child=render.Text(color = "#000000", font = "tom-thumb", content = "TWR")) if TWR_ACTIVE else render.Padding(pad = 2, child=render.Text(font = "tom-thumb", content = "TWR")),
                        ]),
                        render.Stack(children = [
                            render.Image(height = 9, src = ONLINE_2_ICON) if APP_ACTIVE else render.Image(height = 9, src = OFFLINE_2_ICON),
                            render.Padding(pad = 2, child=render.Text(color = "#000000", font = "tom-thumb", content = "APP")) if APP_ACTIVE else render.Padding(pad = 2, child=render.Text(font = "tom-thumb", content = "APP")),
                        ]),
                    ]),
                ]),
            ),
        )
    else:
        return render.Root(
            delay = 5,
            max_age = 15,
            show_full_animation = True,
            child = render.Box(
                padding = 1,
                child =
                render.Column(children = [
                    render.Text(font = "tom-thumb", content = config.str("icao", DEFAULT_ICAO)),
                    render.Text(font = "tom-thumb", content = "DEP:%s|ARR:%s" % (NUM_OF_DEP, NUM_OF_ARR)),
                    render.Text(font = "tom-thumb", content = "--------------------------------"),
                    render.Row(children = [
                        render.Text(font = "tom-thumb", content = "DEL"),
                        render.Image(height = 5, width = 5, src = ONLINE_ICON) if DEL_ACTIVE else render.Image(height = 5, width = 5, src = OFFLINE_ICON),
                        render.Text(font = "tom-thumb", content = "   "),
                        render.Text(font = "tom-thumb", content = "GND"),
                        render.Image(height = 5, width = 5, src = ONLINE_ICON) if GND_ACTIVE else render.Image(height = 5, width = 5, src = OFFLINE_ICON),
                    ]),
                    render.Row(children = [
                        render.Text(font = "tom-thumb", content = "TWR"),
                        render.Image(height = 5, width = 5, src = ONLINE_ICON) if TWR_ACTIVE else render.Image(height = 5, width = 5, src = OFFLINE_ICON),
                        render.Text(font = "tom-thumb", content = "   "),
                        render.Text(font = "tom-thumb", content = "APP"),
                        render.Image(height = 5, width = 5, src = ONLINE_ICON) if APP_ACTIVE else render.Image(height = 5, width = 5, src = OFFLINE_ICON),
                    ]),
                ]),
            ),
        )

def get_cachable_data(url, timeout):
    res = http.get(url = url, ttl_seconds = timeout)

    if res.status_code != 200:
        fail("request to %s failed with status code: %d - %s" % (url, res.status_code, res.body()))

    return res.json()

def get_schema():
    return schema.Schema(
        version = "1",
        fields = [
            schema.Text(
                id = "icao",
                name = "ICAO code",
                desc = "The ICAO code of the airport you want information about.",
                icon = "plane",
            ),
            schema.Toggle(
                id = "styling",
                name = "Styling",
                desc = "Change between two different designs.",
                icon = "plane",
                default = False,
            ),
        ],
    )

